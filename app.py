...
def add_point():
    ...
    return jsonify(
        {"success": True, "point_id": point_id, "total_points": len(solver.points)}
    )
...
def create_wormhole():
    ...
    try:
        ...
        try:
            subprocess.run(
                ["wormhole", "--version"], capture_output=True, text=True, check=True
            )
        ...
        def capture_wormhole_code():
            ...
            try:
                ...
                while (
                    ...
                    and (time.time() - active_transfers[transfer_id]["start_time"])
                    < timeout
                ):
                    time.sleep(1)
                ...
                if process.poll() is None:
                    process.kill()
                    app.logger.warning(
                        f"Transfer {transfer_id} timed out after {timeout} seconds"
                    )
                    active_transfers[transfer_id]["status"] = "timed_out"
                else:
                    active_transfers[transfer_id]["status"] = (
                ...
                try:
                    ...
                    app.logger.error(
                        f"Error cleaning up temp directory for transfer {transfer_id}: {str(e)}"
                    )

                if not wormhole_code:
                    active_transfers[transfer_id]["status"] = "failed"
                    app.logger.error(
                        f"No wormhole code captured for transfer {transfer_id}: {errors}"
                    )
            except Exception as e:
                app.logger.error(
                    f"Error in wormhole thread for transfer {transfer_id}: {str(e)}"
                )
                active_transfers[transfer_id]["status"] = "failed"
                try:
                    process.kill()
        ...
        while time.time() - start_time < 10:  # 10-second timeout for code capture
            if (
                transfer_id in active_transfers
                and "code" in active_transfers[transfer_id]
            ):
                wormhole_code = active_transfers[transfer_id]["code"]
                return jsonify(
                    {"success": True, "code": wormhole_code, "transfer_id": transfer_id}
                )
            time.sleep(0.1)
        ...
        app.logger.error(
            f"Failed to capture wormhole code for transfer {transfer_id} within 10 seconds"
        )
        return jsonify(
            {
                "success": False,
...
def cancel_wormhole():
    ...
    if not transfer_id or transfer_id not in active_transfers:
        return jsonify({"success": False, "message": "Invalid or unknown transfer ID"})
    ...
    try:
        ...
        return jsonify({"success": True, "message": "Wormhole transfer cancelled"})
